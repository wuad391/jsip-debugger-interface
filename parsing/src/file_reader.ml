...
let parse_line line =
  match
    sscanf_opt
      ...
      (fun function_name arguments location ->
         function_name, arguments, location)
  with
  | Some (function_name, arguments, location) ->
    match parse_function function_name, parse_arguments String.split_on_char ',' arguments, parse_location location with 
    | Some function_info, Some args_list, Some location -> (* store stuff *)
